...
export default function DiffusionPage() {
  const models = modelsData.diffusion;
    ...
    <>
      <header className="flex flex-col gap-6 animate-boot">
        ...
        <p className="max-w-2xl text-lg text-gray-400 font-body">
          Open-weight image synthesis and generative diffusion models. Benchmarked on
          FID, CLIP Score, and generation throughput.
        </p>
      </header>
      ...
      <section className="flex flex-col gap-8">
        <div className="flex items-center justify-between border-b border-outline-variant/20 pb-4">
          <h2 className="font-display text-2xl uppercase tracking-wider text-white">
            Diffusion Stream <span className="animate-blink text-primary">_</span>
          </h2>
        ...
        <div className="grid gap-4">
          {models.map((model) => (
            <DiffusionModelCard key={model.id} model={model} />
          ))}
        </div>
      </section>
    </>
...
function DiffusionModelCard({ model }: { model: (typeof modelsData.diffusion)[number] }) {
  return (
    <Link href={`/models/diffusion/${model.id}`} className="bg-surface-low hover:bg-surface-high transition-colors p-6 flex flex-col lg:flex-row gap-6 lg:items-center justify-between group relative overflow-hidden border border-outline-variant/10 no-underline">
      ...
      <div className="flex flex-col sm:flex-row gap-4 lg:items-center shrink-0">
        <div className="bg-surface-highest p-4 rounded-sm flex gap-6 border border-outline-variant/10 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-gray-500 font-mono uppercase">FID</span>
            <span className="font-mono text-primary text-sm">{model.benchmarks.fid}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-gray-500 font-mono uppercase">CLIP</span>
            <span className="font-mono text-secondary-container text-sm">{model.benchmarks.clipScore}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-gray-500 font-mono uppercase">img/s</span>
            <span className="font-mono text-white text-sm">{model.benchmarks.genSpeed}</span>
          </div>
        </div>
